...
#
#   docker build -t snapchat-scraper .
#   docker run --rm -v "$PWD/out:/out" snapchat-scraper \
#     --url nasa,mrbeast --mode spotlight \
#     --out /out/spotlight
#
...
COPY captcha_solver.py env_config.py fingerprint_client.py output_writer.py \
     ...
     proxy_pool.py diff_runs.py snapchat_payload.py ./
